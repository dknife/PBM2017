...
CKangGL::CKangGL() : bStarted(false), bPaused(false), plane(NULL), 
	nPoints(0), nSpheres(0), nHovers(0) {
	mEx = 1;
	mEy = 1;
	mEz = 1;
	mTx = 0;
...
sphere *CKangGL::addSphere(void) {
	sphere *p = new sphere;
	sphereList[nSpheres++] = p;
	return p;
}

Hover *CKangGL::addHover(void) {
	Hover *p = new Hover;
	hoverList[nHovers++] = p;
	return p;
}
...
void CKangGL::removeSpheres(void) {
...
void CKangGL::removeHovers(void) {
...
void CKangGL::display(void) {
	...
	for (int i = 0; i < nSpheres; i++) {
		sphereList[i]->visualize();
	}
	for (int i = 0; i < nHovers; i++) {
		hoverList[i]->visualize();
	}
